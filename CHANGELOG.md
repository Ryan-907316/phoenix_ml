...
- Fixed several tabs (Models, Preprocessing, Uncertainty Quantification, Interpretability, Postprocessing) having no scrollbar, so their content was unreachable on a small screen. Each now scrolls independently; the Home tab keeps its Progress Log and Clear button pinned below a scrollable settings area.
- Added Predicted R² (PRESS) as a Postprocessing CV Scoring Metric, alongside Q²/NSE. Pools every cross-validation fold's held-out residuals before comparing against the target's total sum of squares, rather than averaging per-fold scores like the other metrics. Offered for CV scoring only, as a PRESS value needs the repeated refitting a CV loop already does.
- Fixed R²/Adjusted R²/KGE/Q² silently reporting a meaningless `NaN` (or, for Q², a fabricated `0.0`) as the CV "Mean Score" whenever a fold had fewer than two test samples. Now raises a clear error and points to Predicted R² (PRESS) as the correct alternative.
- Fixed settings for Uncertainty Quantification, Interpretability, Hyperparameter Optimisation, and Cross-Validation being validated on every run regardless of whether that step was enabled, so a setting for a step you weren't running could still block you. Now validated only when the step is enabled, or run directly. Added an upfront hint for FAST's N > 4×M² requirement instead of only surfacing it as an error afterwards.
- Added Model Cards: a one-page PDF per target, saved alongside the other Model Deployment artefacts, documenting what it predicts, its headline metrics, and a short list of known limitations for a reader who may only have this one file. Later extended with the cross-validated CV score for whichever scoring metric was selected, an intended-use statement, the full list of input features, the deployed model's actual hyperparameter values, and the preprocessing scaler used.
- Fixed the Reproducibility Metadata (.json) recording Hyperparameter Optimisation, Uncertainty Quantification, and Interpretability settings even when those steps never ran, falsely implying they were used. Now recorded as `null` for any step that did not actually run.
- Fixed a Model Card showing no uncertainty coverage information at all for a run with HPO disabled and only "UQ Before HPO" computed. It now falls back to that data, but only when the deployed model is the same untuned model both stages describe.
- Fixed the "rows removed during cleaning" note on Model Cards and the report's Executive Summary being unreachable in normal use, since the cleaning summary was discarded before the report could read it.
- Added two new Preprocessing split methods: Stratified (preserves the target's distribution across train/test, useful on a small or skewed dataset; needs a single target column) and Systematic (evenly spaced rows via a fixed stride).
- Added ElasticNet as a selectable model, tuned over `alpha` and `l1_ratio` like every other model.
- Consolidated the Home tab's Uncertainty Quantification and Interpretability rows into a single grouped block with Before/After HPO sub-rows, instead of two separate top-level rows each. "After HPO" greys out when Hyperparameter Optimisation's own checkbox is off, and "Before HPO" relabels to "Default Hyperparameters" in that case, since there is no "after" to contrast it against.
- Fixed "Before HPO"/"Default Hyperparameters" labelling being inconsistent across the report, the Excel export, and the pipeline table in several edge cases (HPO disabled outright, HPO stopped mid-run, a step force-run out of its usual order via its own Run button). Every label is now derived from whether HPO actually produced results, rather than from whichever setting happened to be ticked at the time.
- Fixed the Executive Summary's uncertainty-quantification sentence silently omitting GP Posterior from its list of methods, even when enabled and shown throughout the rest of the report.
- Fixed several settings (GP Posterior, Monotonicity Constraints, Hyperparameter Optimisation's per-method fields, Bootstrap/Conformal UQ fields, and the Dataset Cleaning Export button) silently re-enabling themselves moments after being correctly greyed out, whenever an unrelated step started or finished elsewhere in the app.
- Fixed the Target vs Target plot only ever showing the first two target variables, regardless of how many targets a dataset has. Now shows every pair, arranged three per row.
- Added `hypot`, `mod`, `min`, and `max` as two-argument functions in the Physics Modelling expression editor, alongside the existing `atan2`.
- A wide batch of smaller correctness fixes found during a full pre-release audit: crashes generating the PCA plot and the Distance Correlation matrix on a small or feature-less dataset; a crash (or silently dropped results) when GP Posterior succeeds for one target but fails for another; monotonicity constraints not being reapplied to the "After HPO" model used by Uncertainty Quantification and Interpretability; Random Search-won integer hyperparameters showing a spurious ".0" in the report; a numeric "Year" column being misclassified as a date and corrupted if cleaned; the Data Preview grid's highlighting not matching the Apply panel's actual settings; the DFFITS threshold using a slightly incorrect formula; and missing input validation on two of the three Hyperparameter Optimisation backends. See tests/ISSUES.md for full detail on each.
- Fixed a non-ASCII character in the CLI's `--help` text rendering as mojibake in some terminal encodings, the same class of issue already fixed elsewhere in the package's console output.
- Tightened the numpy dependency bound from `< 3` to `< 2.5`. numba (pulled in transitively by `dcor` and `shap`) caps itself at `numpy < 2.5`, so a fresh install could otherwise resolve a numpy version too new for numba, forcing pip to fall back to a numba/llvmlite combination too old to support the Python versions this package requires.
